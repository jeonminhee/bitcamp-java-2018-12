...
<%-- 자바 원시 타입과 문자열을 제외한 속성의 값은 지정할 수 없다.
     따로 처리해야한다. 
<jsp:setProperty  name="b3" property="createdDate" value="2019-04-08"/>
--%>
...
=> 주요 속성
   ...
   class
    ...
    - 반드시 패키지 이름을 포함하여 클래스명(fully-qualified name; FQName)을 지정해야한다.
      <%@ page import="..."%>를 선언해도 소용없다.
    - 객체를 꺼내는 경우 레퍼런스의 타입으로도 사용된다.
    - 객체를 생성할 때도 사용할 수 있기 때문에 반드시 콘크리트(concrete) 클래스명이어야한다.
