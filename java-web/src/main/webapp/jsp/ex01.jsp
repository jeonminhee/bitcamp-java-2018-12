...
2) 서블릿 컨테이너가 실행
   ...
   2.3 없으면,
     ...
     2.3.4 그리고 그 서블릿 객체를 호출한다. - service() 호출
   2.4 JSP 파일이 변경된 상태라면,
     2.4.1 다시 "2.3"을 반복한다.
3) 서블릿 컨테이너 ==> 웹 브라우저에게 응답
   - 서블릿 실행 결과를 응답
 ...
 - JSP 엔진의 역할을 JPS 파일을 분석하여 서블릿 클래스를 생성하는 것이다.
 - 즉 JSP 파일이 직접 실행되지 않는다!
 
 JSP
 - 자바 서블릿 클래스를 만드는 재료로 사용된다.
 - 그래서 서블릿 클래스를 만드는 "틀"이라 해서 "템플릿(Template)"라 부른다.
 - JSP를 템플릿 기술이라 부르기도 한다.
 
 JSP 공부법
 - JSP를 작성할 때 사용하는 특정 태그들이 어떤 JAVA 코드를 생성하는지 이해하는 것이 중요하다.
