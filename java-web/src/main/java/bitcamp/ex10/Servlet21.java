...
public class Servlet21 extends HttpServlet {
  ...
  protected void service(
    ...
    // 쿠키 유효기간
    // => 쿠키의 사용범위를 지정하지 않으면 쿠키를 발행한 URL 범위에 한정된다.
...
Set-Cookie: v2=bbb; Path=/java-web/ex10/a <--- 사용범위가 /java-web/ex10/a로 제한되었다.
Set-Cookie: v3=ccc; Path=/java-web
Content-Type: text/plain;charset=UTF-8
Content-Length: 37
Date: Wed, 03 Apr 2019 02:05:55 GMT
     */
    ...
    // 왜 쿠키의 경로를 적을때 웹 애플리케이션 루트(컨텍스트 루트)까지 적나요?
    // => 쿠키 경로는 서블릿 컨테이너가 사용하는 경로가 아니다.
    // => 웹 브라우저가 사용하는 경로이다.
    // => 웹 브라우저에서 '/'은 서버 루트를 의미한다.
    // => 따라서 웹 브라우저가 사용하는 경로를 지정할 때는 조심해야한다.
    //    '/' 가 서버 루트를 의미하기 때문이다.
    // => 그래서 쿠키의 경로를 지정할 때는 웹 애플리케이션 루트(컨텍스트 루트)를 정확하게 지정해야한다.
