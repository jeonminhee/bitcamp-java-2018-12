...
public class Servlet01 extends HttpServlet {
  ...
  protected void service(
    ...
    // 어디에 놓든 상관이 없다. 응답헤더가 먼저 호출된다.
    // 위에서 벌써 클라이언트에게 응답을 했는데 어떻게 응답 헤더를 출력할 수 있나요?.?
    // => 잊지말자! out.println이 출력한 것은 출력스트림 버퍼에 보관되어 있다.
    //    따라서 아직 클라이언트에게 응답한 상태가 아니다.
    //    그래서 다음과 같이 출력을 한 후에 응답헤더 값을 추가하거나 변경할 수 있는 것이다.
    ...
    //    따라서 일단 클라이언트에게 응답을 하면 다음에 코드는 적용되지 않는다.
    ...
    for(int i = 0; i < 200; i++) {
      // 약 50 바이트 씩 100번 출력하면 아직 버퍼에 차지 않았기 때문에
      // 클라이언트에 출력되지 않는다.
      // 따라서 반복문 아래에 있는 응답헤더 설정이 유효하다.
