...
public class Test10 {
  ...
  public static void main(String[] args) {
    ...
    // 저장할 때,
    // => Key 객체의 hashCode() 리턴값으로 위치를 계산하여 저장한다.
    //
    // 값을 꺼낼 때,
    // => Key 객체의 hashCode() 리턴값으로 위치를 계산한다.
    // => Key 객체의 equals()의 리턴값으로 같은 Key 인지 검사한다.
    //
    // 결론,
    // => k2와 k3는 비록 인스턴스가 다르더라도 인스턴스 필드의 값은 같다.
    // => 그러나 hashCode()가 다르기때문에 위치 계산 결과가 달라진다.
    // => 또한 equals()의 리턴값이 false이기 때문에 다른 key로 인식한다.
    // => 그래서 k2로 저장한 값을 k4로 꺼낼 수 없다.
    //
    // 해결책?
    // => 인스턴스가 다르더라도 인스턴스 필드의 값이 같을 때는
    //   hashCode()의 리턴값이 같게 하라.
    //    또한 equals()의 리턴값이 true가 되게하라.
  }
}
