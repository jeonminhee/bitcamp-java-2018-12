...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    // => 이 해시값을 가지고 저장 할 위치를 결정한다.
    //
    map.put("aaa", new Student("홍길동", 20));
    // 1) "aaa" String 객체에 대해 hashCode()를 호출하여 해시값을 얻는다.
    ...
    // 맵에서 값 꺼내기
    // => 저장 할 때 사용한 key를 가지고 꺼낸다.
    System.out.println(map.get("bbb"));
    // 1) 키(key) 객체에 대해 hashCode()를 호출한다.
    // 2) 해시코드(hashCode())의 리턴 값을 가지고 데이터를 찾을 위치를 결정한다.
    // 3) 해당 위치에 있는 key 객체에 대해 equals()를 호출하여 리턴값을 확인한다.
    // 4) equals()의 리턴값이 true라면 같은 key로 간주하여 해당 위치의 값을 꺼낸다.
