...
public class Server1 {
  ...
  public static void main(String[] args) {
    try(Scanner keyboard = new Scanner(System.in);
      ...
      System.out.println("클라이언트의 기다리고 있음. . . ");
      ...
      try (
        ...
        InputStream in = socket.getInputStream()){
          ...
          // 따라서 서버는 한 줄의 문자열을 읽은 후에 응답한다.
          
          // 서버에서 한 줄의 데이터를 읽기 전에는 클라이언트의 println()이 리턴하지 않음을
          // 증명하기 위해 클라이언트가 보낸 데이터를 읽지 않도록 잠시 실행을 중단한다.
          System.out.println("데이터를 읽기 전에 잠깐!!!!");
          keyboard.nextLine(); // 사용자가 콘솔창에서 엔터를 칠 때 까지 리턴하지 않는다.
          
          // => 클라이언트 한 줄의 문자열을 보낼 때 까지 리턴하지 않는다.
          // => 한 줄의 문자열 ? 문자열 끝에 줄바꿈 코드(Line Feed; 0x0A)를 만날 때 까지. 
          int request = in.read(); // blocking 모드로 작동한다.
          System.out.println(request);
          
          // 서버가 데이터를 보내지않으면 클라이언트의 nextLine()은 리턴하지 않는다.
