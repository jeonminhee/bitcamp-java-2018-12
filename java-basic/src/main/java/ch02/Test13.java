...
public class Test13 {
  public static void main(String[] args) {
    //현재 날짜 미 시각 정보를 생성한다.
    //java.lang 패키지 멤버를 사용할 때는 그냥 이름을 지정하면 된다.
    // 그 외 다른 패키지의 멤버를 사용할 때 반드시 패키지 이름을 지정해야한다.
    java.util.Date today = new java.util.Date();

    //%t[날짜 및 시간 옵션]
    //날짜 및 시간 옵션
    //Y : 날짜 및 시간 데이터에서 년도를 추출하여 4자리로 표현
    //y : 날짜 및 시간 데이터에서 년도를 추출하여 2자리로 표현
    System.out.printf("%1$tY, %1$ty\n", today);

    //B : 날짜 및 시간 데이터에서 월 추출하여 전체 이름으로 표현 ex) January
    //b : 날짜 및 시간 데이터에서 월 추출하여 전체 이름으로 표현 ex) Jan
    System.out.printf("%1$tB, %1$tb\n", today);

    //m : 날짜 및 시간 데이터에서 월 추출하여 2자리 숫자로 표현 ex) 12, 01
    System.out.printf("%1$tm\n", today);

    //d : 날짜 및 시간 데이터에서 일을 추출하여 2자리 숫자로 표현 ex) 01, 22
    //e : 날짜 및 시간 데이터에서 일을 추출하여 1자리 숫자로 표현 ex) 1, 22
    ...
    //N : 날짜 및 시각 데이터에서 시각을 추출하여 나노초을 표현
    System.out.printf("%1$tS %1$tL %1$tN\n", today);

    //p : 오전 오후 출력하기
    //      소문자 t를 사용하면 am, pm으로 출력하고,
    //      대문자 T를 사용하면 AM, PM으로 출력한다.
    //      한글은 대소문자가 없기 떄문에 의미없음.
