...
// 패키지 멤버 클래스
// => 단독으로 선언하는 클래스이다.
// => 물론 이 예제 처럼 한 파일에 여러개의 클래스를 선언 할 수 있지만
...
public class Test09 {
  ...
  // 중첩 클래스
  // => 클래스 안에 소속된 클래스이다.
  // => 보통 특정 클래스 안에서만 사용될 경우 중첩클래스로 정의한다.

  // 스태틱 중첩 클래스(static nested class) = top level nested class
  // => 이 클래스를 소유하고 클래스 뿐만 아니라 다른 클래스도 사용할 수 있다.
  static class Score2 {
    String name;
    int kor;
    int eng;
  ...
  public static void main(String[] args) {
    ...
    // 로컬 클래스(local class)
    // => 특정 메소드 안에서만 사용되는 클래스인 경우 로컬 클래스로 정의한다.
    class Score4 {
      String name;
      int kor;
      int eng;
    ...
    // => 클래스를 정의 할 때 수퍼 클래스나 인터페이스를 지정해야한다.
    // => 클래스를 정의할 때 new 연산자를 사용하여 즉시 인스턴스를 생성해야한다.
  ...
  static void m1() {
    ...
    // 같은 패키지가 아니라도 공개된 패키지멤버는 누구라도 사용할 수 있다.
    Score1 s1 = new Score1();
    
    // 같은 static 멤버인 static 중첩 클래스를 사용할 수 있다.
  ...
  void m2() {
    ...
    // 논-스태틱 멤버(인스턴스멤버)는 스태틱 멤버를 사용할 수 있다.
    Score2 s2 = new Score2();
    
    // 인스턴스 멤버는 인스턴스 멤버를 사용할 수 다.
