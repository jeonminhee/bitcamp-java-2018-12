...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    //    $ gradle eclipse를 실행하여 라이브러리 파일을 다운로드 받는다.
    //    이클립스에서 프로젝트 정보를 refresh한다.
    // ApplicationContext 인터페이스
    // => 스프링 IoC 컨테이너의 사용규칙을 정의한 인터페이스이다.
    // => 모든 스프링 IoC 컨테이너는 이 규칙에 따라 IoC 컨테이너가 정의되어있다.
    //
    ...
    //    물론 JVM이 클래스파일을 찾을 때 JVM의 기본 경로(JAVA_HOME/lib)를 가장 먼저 뒤진다.
    ...
    // 2) 운영체제의 파일 시스템에서 설정 파일을 찾는 IoC Container
    // => 설정 파일 경로를 지정할 때 직접 파일 경로를 지정해야한다.
    // => 주의!
    //    설정 파일 시스템 경로를 지정할 때 URL 형식으로 지정해야 한다.
