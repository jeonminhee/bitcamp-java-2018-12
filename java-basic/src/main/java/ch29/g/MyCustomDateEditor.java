...
// Spring IoC Container에서 사용 할 프로퍼티 에디터 만들기
...
public class MyCustomDateEditor extends PropertyEditorSupport {
  ...
  // Spring IoC Container는 Spring을 java.sql.Date 클래스로 바꾸기 위해 이 메서드를 먼저 호출한다.
  ...
  public void setAsText(String text) throws IllegalArgumentException {
    ...
    // 이 값을 분석하여 우리가 원하는 java.sql.Date 객체로 바꾸면된다.
    Date date = Date.valueOf(text);
    
    // String값을 java.sql.Date 객체로 바꾼 다음에 
    // 내부에 보관한다.
    // => 이렇게 setValue()로 보관된 값을 
    //    Spring IoC Container는 getValue()를 호출하여 꺼내쓴다.
    this.setValue(date);
  }
