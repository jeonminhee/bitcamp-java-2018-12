...
//   - 'gradle eclipse' 실행하여 라이브러리를 다운받고, 이클립스 설정 파일에 등록한다.
...
public class Test01 {
  ...
  public static void main(String[] args) throws Exception {
    ...
    // 1) mybatis 설정파일을 읽을 때 사용할 도구를 준비한다.
    // => 파일 입력 스트림 객체 준비
    // => FileInputStream 객체를 직접 생성해도 되지만,
    //    직접 생성하려면 설정 파일의 전체 경로를 정확하게 지정해야한다.
    //    예) c:/Users/user/git/bitcamp-2018-12/java-basic/src/main/java/ch26/a/mybatis-config.xml
    //    만약 경로가 바뀌면 소스의 경로를 다시 변경해야하는 문제가 있다.
    // => 그래서 mybatis 설정 파일이 자바 클래스파일(.class)처럼 CLASSPATH에 있다면
    //    Resources.getResourceAsStream()을 호출하여 입력 스트림을 준비하는 것이 더 편하다.
    //
    String resource = "ch26/a/mybatis-config.xml";
    InputStream inputStream = Resources.getResourceAsStream(resource);
    
    // 만약 Resouces.getResourceAsStream()을 사용하지 않는다면 다음과 같이
    // 직접 FileInputStream을 생성해야한다.
