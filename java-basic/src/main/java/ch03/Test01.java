...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    f1 = 98765.43f;
    f2 = 98765.43456789f; //정수와 달리 유효 자릿수를 넘어 가는 경우 짤려서 저장된다.
    ...
    System.out.println(f3); // 실제 출력 값 : 9888.889
    
    
    double d3;
    d3 = f1 + f2; // 기대하는 값 = 9888.88867
    System.out.println(d3); // 실제 출력 값 : 9888.888671875
        
    // 8바이트 부동소수점을 담을 메모리 준비
    double d1; // 주의! 8바이트 부동소수점을 표현할 때는 f, F 접미사를 제거하라.
    double d2;
    ...
    System.out.println(d2); // 유효자릿수가 15자리까지 대부분 짤리지 않고 들어간다.
    ...
    bool1 = true;
    bool2 = false;
    //bool3 = 1; // 컴파일 오류! true, false외의 다른 값은 저장할 수 없다.
    ...
    c1 = 44032; // 4바이트 정수 리터럴이더라도 0 ~ 65535값이라면 
                // char 메모리에 저장하는 것을 허락한다.
    c2 = 0xac00;
    c3 = '가'; // ''의 리턴 값은 2바이트(0 ~ 65535) 유니코드 정수값이다. 
    System.out.println(c1); // 변수 자체가 char이기 때문에 (char)을 붙이지 않아도 된다.
 ...
 * - 즉 값을 저장할 메모리의 위치와 크기를 결정하고 그 메모리에 이름을 부여한다.
