...
public class Test01 {
  ...
  // => 구현하기 편하다.
  // => 그런데 다중상속이 불가능하기 때문에 다른 클래스를 상속받을 수 없다.
  ...
  public static void main(String[] args) {
    ...
    // 스레드 실행
    // 1) 스레드의 서브 클래스는 그냥 인스턴스를 만들어 start()를 호출한다.
    ...
    // 스레드에게 CPU 사용을 배분할 때 순서대로 하지않는다.
    ...
    //
    // 우선 순위로 조정하면 되지않나요?
    // => Windows OS의 경우 우선순위(priority) 값이 실행 순서나 실행 횟수에 큰 영향을 끼치지않는다.
    //      그래서 우선 순위의 값을 조정하여 스레드의 실행 횟수를 조정하려 해서는 안된다.
    // => 왜? OS에 따라 실행정책이 다르기 때문이다.
