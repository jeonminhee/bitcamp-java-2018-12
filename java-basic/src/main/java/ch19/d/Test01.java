...
public class Test01 {
  ...
  public static void main(String[] args) {
    ...
    //    이런 식으로 점(.)을 이용하여 하위 필드로 지정하는 표기법을 OGNL이라 한다.
    // => static nested class를 사용하여 더 극단적으로 분류 값을 OGNL 표기법으로 사용할 수 있다.
    // => 다음과 같이 하나의 이름을 사용하는 대신에 점(.)을 사용하여 하위 프로퍼티를 접근하는 방식은
    //    코드를 읽기 쉽고 유지보수하기 쉽게 도와준다.
    // => 이렇게 클래스 이름으로 필드처럼 사용 할 수 있어 보기가 쉽다.
    p.category = Category.computer.mouse.bluetooth;
    
    // static nested class를 이런 식으로 적용한 대표적인 예가
    // 안드로이드가 컴파일러가 자동생성하는 R.java이다.
    //
  }
}
