...
class My4 {
  // 클래스 필드 = 스태틱 필드(변수)
  // => 클래스가 로딩될 때 생성되는 변수
  // => 클래스 필드도 인스턴스 필드처럼 기본 값으로 자동 초기화된다.
  //    정수(byte, short, int, long, char) => 0
  //    부동소수점 => 0.0
  //    논리 => false
  //    레퍼런스 => null
  //  => 참고! 로컬 변수는 절대로 자동 초기화되지 않는다.
  static int a;
...
public class Test04 {
  public static void main(String[] args) {
    ...
    // 클래스가 로딩 될 때? 클래스 멤버(필드, 메서드)를 사용할 때!
    // - 클래스 필드를 사용할 때
    // My4.a = 300;
...
2) 클래스에 바이터코드의 유효성을 검사한다.
3) 클래스 내부에서 사용하는 모든 이름에 대해 (변수명, 메서드명, 클래스명 등) 목록을 준비한다.
4) 클래스 필드를 찾아 생성한다.
