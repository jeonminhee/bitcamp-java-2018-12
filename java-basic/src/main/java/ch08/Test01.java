...
class My1 {
  // My1 인스턴스(데이터타입의 값)을 다룰 때 사용할 값이라면 따로 떼어놓지말고,
  ...
  // => 클래스는 JVM마다 딱 한번만 로딩되기 때문에 <클래스 필드>도 딱 한번만 생성된다.
  //    그래서 메모리를 절약할 수 있따.
  // => 클래스 필드를 만드는 문법 : 
  //        static 데이터타입 변수명;
  // => 클래스 필드 사용법 : 
  ...
  // new 명령을 실행할 때 heap 메모리에 생성되는 변수이다.
  // 이런 변수를 <인스턴트 필드>라고 부른다. 인스턴스를 만들때 생성되는 변수를 인스턴스 필드라고 한다.
  String name;
  int age; 
  String userType;
}
