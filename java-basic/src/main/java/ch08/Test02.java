...
public class Test02 {
  public static void main(String[] args) {
    ...
    // 3) main()에 메서드에 선언된 로컬 변수를 "JVM 스택 영역"에 생성한다.
    //          -args 변수를 스택 영역에 생성한다.
    // 4) main()의 코드를 실행한다.
    //      - My2.calss를 Method Area에 로딩한다.
    //        - My2의 클래스를 필드 Method Area에 생성한다.
    //          - System.out.println(My.manager);를 실행한다.
    //          - My2 클래스가 이미 로딩되었기 때문에 다시 로딩하지 않는다.
    //          - System.out.println(My.member);를 실행한다.
...
/*
# 클래스 로딩
- 외부 저장장치(예 : HDD, USB메모리, DVD-ROM 등)에 있는 
 .class파일을 JVM이 관리하는 메모리로 로딩하는 것.
...
2) JVM Stack
  - 각 스레드가 개인적으로 관리하는 메모리영역이다.
  - 스레드에서 메소드를 호출할 때 메서드의 로컬 변수를 이 영역에 만든다.
  - 메서드가 호출될 때 그 메서드가 사용하는 로컬변수를 프레임에 담아 만든다.
  - 메서드 호출이 끝나면 그 메서드가 소유한 프레임이 제거된다.
3) Method Area
  - JVM이 실행하는 바이트코드(.class 파일)를 두는 메모리 영역이다.
  - 즉 클래스 코드가 이 영역에 놓이는 것이다.
  - JVM은 코드를 실행할 때 이 영역에 있는 놓인 명령어를 실행하는 것이다.
  - 주의! Heap에는 개발자가 작성한 명령어가 없다!
  - 개발자가 작성한 클래스, 메서드 등 이런 코드들이 이 영역에 놓이는 것이다. 
  - 스태틱 필드를 이 영역에 생성한다.
  
 # 클래스 로딩
 - 클래스의 코드를 사용하려는 시점에 메모리(Method Area 영역)에 로딩된다.
 ...
 - 스태틱 멤버(필드와 메서드)를 사용할 때
   예) My2.member = 200;
   예) System.out.println(My2.manager);
   예) Integer.parseInt(..);
 - new 명령을 사용하여 인스턴스를 생성할 때
   예) new My2();
 - 주의!
   => 레퍼런스 변수를 선언할 때는 클래스를 로딩하지않는다.로딩하지않는다.로딩하지않는다.로딩하지않는다.
   My2 obj1;
