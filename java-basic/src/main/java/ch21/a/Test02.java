  ...
  public static void main(String[] args) {
    ...
    try {
      ...
      for(int i = 0; i < 1000; i++) {
        ...
        // 만약 Heap 영억의 메모리가 모두 사용된다면
        // 더 이상 배열을 생성할 수 없기 때문에 "시스템 예외"가 발생 할 것이다.
      }

    } catch (Error err) {
      // 예외가 발생하면 이 블록이 실행된다.
      // 이 블록에서 예외를 받으면 JVM에게 예외가 보고되지 않는다.
      // => 자바버츄얼머신(JVM)에게 예외 상황이 보고되지 않으면 즉시 멈추지않는다.
      //
    ...
    // ㅁ catch 블록에서 예외를 받았다고 해서 heap의 메모리 부족 상태가 해결되는 것은 아니다.
