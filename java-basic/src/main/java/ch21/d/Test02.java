...
public class Test02 {
  ...
  public static void main(String[] args) {
    ...
    // Exception 계열(RuntimeEX 제외)의 예외가 발생하는 메서드를 호출 할 때는
    // 반드시 예외처리를 해야 한다.
    // try ~ catch로 처리하든 throws 절을 메서드에 선언부에 붙이든 둘 중 하나는 해야한다.
    // 
    ...
    // int값을 리턴하는 메서드가 오류를 알리는 방법
    // => 이상한 값을 리턴한다.
    //
    // int result = divide(1000, 7); // OK
    // int result = divide(1000, 0); // Ok, 예외임을 알 수 있다.
    int result = divide(19191919, -1); // 정상적인 나누기임에도 불구하고
    if(result == -19191919) { // 리턴값이 -19191919이기 때문에 오류로 판단한다.
      System.out.println("나누기 오류입니다."); // 이것이 리턴값으로 오류를 전달할 때의 문제이다.
    } else {
  ...
  // 2) 리턴 값으로 예외를 보고하기
  // => ㄴ C 프로그래밍 시절에는 주로 리턴 값으로 예외 상황을 보고하였다.
  ...
  static int divide(int a , int b) {
    if(b == 0) {
      // 호출자에게 어떤 값으로 알려줘야 오류를 전달 할 수 있을까?
