...
// => 물론 처음에는 ThreadLocal에 SqlSession이 없기 것이기 때문에
//    원래의 SqlSessionFactory 구현체를 이용해 SqlSession을 만들 것이다.
// => 프록시패턴?
//    프록시는 반드시 원본 객체와 같은 규칙을 따라야한다.
// => 트랜잭션 지원하기
...
public class SqlSessionFactoryProxy implements SqlSessionFactory {
  ...
  public SqlSessionFactoryProxy(SqlSessionFactory original) {
    // Mybatis에서 제공하는 SqlSessionFactory 구현체를 필드에 보관해두었다가
    // 외부에서 요청이 들어오면 이 객체를 사용하여 SqlSession객체를 만들어준다.
    this.original = original;
  }

  // 트랜잭션 관리자가 트랜잭션을 시작시키면
  // 기본적으로 auto commit이 false인 SqlSession 객체를 미리 스레드에 보관한다.
  ...
  // 스레드의 SqlSession이 보관되어 있으면 그 객체를 리턴하고
  // 없으면 원래대로 SqlSession객체를 만들어 리턴한다.
