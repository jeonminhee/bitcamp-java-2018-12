...
public class ApplicationContext {
  ...
  public ApplicationContext(Class<?> configClass) throws Exception {
    ...
    if(componentScan != null) {
      String[] packages = componentScan.basePackage();
  ...
  private Object callFactoryMethod(
      Object obj, Method factoryMethod, List<Method> factoryMethods, String indent) throws Exception {
    
    // System.out.println(indent +  "===>" + factoryMethod.getName());
    
    // 1) m 메서드에서 호출할 때 필요한 파라미터 정보를 알아낸다.
    // ex) m(BoardDao, MemberDao, txManager)
    //     => 파라미터 타입 배열 : {BoardDao.class, MemberDao.class, txManager.class}
    Class<?>[] paramTypes = factoryMethod.getParameterTypes();

    // 파라미터 값을 담을 배열을 준비한다.
    // ex) {new BoardDao(), new MemberDao(), new txManager()}
    ...
    for(int i = 0; i < paramTypes.length; i++) {
      ...
      if(paramValue == null) {
        ...
        // 그 메서드를 호출하기위해 파라미터 값이 필요했는데
        // 그 파라미터 값이 없어서 메서드 호출에 실패했다면
        // factory 메서드 호출을 포기한다.
        return null;
      }
    ...
    // 팩토리 메서드를 호출할 수 있따면 팩토리 메서드 목록에서 제거한다.
    ...
    // 팩토리 메서드가 생성한 객체를 저장할때 
    // 애노테이션에 빈의 이름이 저장되어 있다면 그 이름을 사용하고
    // 없다면 메서드 명을 사용한다.
    Bean beanAnno = factoryMethod.getAnnotation(Bean.class);
    beanContainer.put(
        beanAnno.value().length() > 0 ? beanAnno.value() : factoryMethod.getName(), bean);
    // System.out.println(indent + "   : " + bean.getClass().getSimpleName() + " 객체 보관됨 ");
