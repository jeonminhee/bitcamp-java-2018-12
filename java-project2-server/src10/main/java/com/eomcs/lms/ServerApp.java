// 10단계: 멀티스레드 상황에서 DB 커넥션을 공유할 때의 문제점과 그 해결책1
//
...
//     => 또한 DAO 객체를 생성할 때 생성자에게 파라미터 값으로 커넥션 객체를 넘겨주지 않는다.
//  4) AbstractCommand 변경
//     => execute() 호출이 완료되었을때 commit() 또는 rollback()을 호출하지 않는다.
...
//     => 이때 첨부파일 두 개를 입력한 후 세번째 입력할 때 예외가 발생하도록
//        긴 이름의 파일명을 입력한다.
...
public class ServerApp {
  ...
  // 바깥 클래스의 인스턴스 필드를 사용해야한다면 Inner 클래스로 정의하라!
