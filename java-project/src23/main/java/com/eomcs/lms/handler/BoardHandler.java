...
public class BoardHandler {
  ...
  public BoardHandler(Scanner keyboard, List<Board> list) {
    ...
    // 다음과 같이 이 클래스가 사용 할 List 객체를 LinkedList로 결정해버리면,
    // List 사용 규칙을 따르는 다른 객체로 쉽게 변경할 수 없다.
    // 다른 List 객체로 변경하려면 이 클래스의 생성자 코드를 바꿔줘야한다.
    // 어떤 기능을 변경할 때마다 소스 코드를 변경하는 방식은 유지보수에 좋지않다.
    // 해결책?
    ...
    //
    // this.list = new LinkedList<>(); // 이클래스에서 의존 객체를 결정하지말자.
