...
<ul class="navbar-nav mr-auto">
  ...
  <div id="bit-login-state">
    <c:if test="${empty loginUser}">
      <a class="btn btn-success btn-sm" href='${contextRootPath}/app/auth/form' role="button" aria-pressed="true">로그인</a>
      </c:if>
      <c:if test="${not empty loginUser}">
        <img src='${contextRootPath}/upload/member/${loginUser.photo}' style='height: 20px;'>${loginUser.name}
      <a class="btn btn-dark btn-sm" href='${contextRootPath}/app/auth/logout' role="button" aria-pressed="true">로그아웃</a>
    </c:if>
  </div>
