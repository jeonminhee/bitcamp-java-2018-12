...
-- 강의실
DROP TABLE IF EXISTS cals RESTRICT;

-- 주소
DROP TABLE IF EXISTS addr RESTRICT;

-- 강의실사진
DROP TABLE IF EXISTS cals_phot RESTRICT;
...
-- 강의실
ALTER TABLE cals
  ADD CONSTRAINT PK_cals -- 강의실 기본키
    PRIMARY KEY (
      clas_id -- 강의실번호
    );

-- 강의실 유니크 인덱스
CREATE UNIQUE INDEX UIX_cals
  ON cals ( -- 강의실
    loc_id ASC, -- 지점번호
    name ASC    -- 교실명
  );
...
-- 강의실사진
CREATE TABLE cals_phot (
  cals_phot_id INTEGER      NOT NULL COMMENT '강의실사진번호', -- 강의실사진번호
  clas_id      INTEGER      NOT NULL COMMENT '강의실번호', -- 강의실번호
...
-- 강의실사진
ALTER TABLE cals_phot
  ADD CONSTRAINT PK_cals_phot -- 강의실사진 기본키
    PRIMARY KEY (
      cals_phot_id -- 강의실사진번호
    );

ALTER TABLE cals_phot
  MODIFY COLUMN cals_phot_id INTEGER NOT NULL AUTO_INCREMENT COMMENT '강의실사진번호';
...
-- 강의실
ALTER TABLE cals
  ADD CONSTRAINT FK_loc_TO_cals -- 지점 -> 강의실
    FOREIGN KEY (
      loc_id -- 지점번호
    )
    REFERENCES loc ( -- 지점
...
-- 강의실사진
ALTER TABLE cals_phot
  ADD CONSTRAINT FK_cals_TO_cals_phot -- 강의실 -> 강의실사진
    FOREIGN KEY (
      clas_id -- 강의실번호
    )
    REFERENCES cals ( -- 강의실
...
-- 교육과정
ALTER TABLE cours
  ADD CONSTRAINT FK_cals_TO_cours -- 강의실 -> 교육과정
    FOREIGN KEY (
      clas_id -- 강의실번호
    )
    REFERENCES cals ( -- 강의실
