...
//    => 예) BoardDao interface 정의
// 2) 기존 클래스를 인터페이스를 구현하도록 변경한다.
//    => 예) BoardDaoImpl 클래스로 변경
// 3) 서버 쪽에서 BoardDaoImpl 객체를 대행 할 클래스를 만든다.
//    => 예) BoardService의 이름을 BoardDaoSkel로 변경한다.
// 4) 클라이언트 쪽에서 BoardDaoImpl 객체를 대행 할 프록시 클래스를 만든다.
//    => 예) BoardAgent 클래스를 BoardDaoPorxy 클래스로 변경한다.
//        BoardDaoProxy 클래스는 BoardDaoImpl 클래스와 마찬가지로 BoardDao 인터페이스를 구현한다.
//           패키지 명도 agent 대신 proxy로 변경한다.
